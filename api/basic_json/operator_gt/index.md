...
1. Compares whether one JSON value `lhs` is greater than another JSON value `rhs` according to the following rules:
   ...
   - Otherwise, returns the result of `!(lhs <= rhs)` (see [**operator\<=**](https://json.nlohmann.me/api/basic_json/operator_le/index.md)).

1. Compares whether a JSON value is greater than a scalar or a scalar is greater than a JSON value by converting the scalar to a JSON value and comparing both JSON values according to 1.
