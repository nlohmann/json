...
TEST_CASE("CBOR regressions")
{
    ...
    {
        // 4200 nested CBOR arrays of length 1 (0x81 each) - comfortably past
        // max_depth (4096, see binary_reader.hpp); the (now iterative,
        // heap-stack based) parser must reject this with a clean parse_error
        // once the nesting exceeds the depth limit, rather than exhausting
        // the native call stack.
        std::vector<uint8_t> vec(4200, 0x81);
        vec.push_back(0x00);
